...
RegisterKeyMapping("VioceChange", "Change Proximity", "Keyboard", mumbleConfig.controls.proximity.key)
RegisterKeyMapping("switchme", "Used to talking on radio", "Keyboard", mumbleConfig.controls.radio.key)
-- Functions
...
Citizen.CreateThread(function()
	-- Check if the correct control is pressed (mumbleConfig controls)
	if IsControlJustPressed(0, mumbleConfig.controls.radio.key) then            
		-- Check if the animation is already playing
		local isPlayingAnim = IsEntityPlayingAnim(playerPed, "anim@cop_mic_pose_002", "chat_mic", 3)

		-- Ensure player is unarmed before starting the animation
		...
		while not HasAnimDictLoaded("anim@cop_mic_pose_002") do
			Citizen.Wait(1)
		end

		-- If the animation is not playing, start it
		if not isPlayingAnim then
			TaskPlayAnim(playerPed, "anim@cop_mic_pose_002", "chat_mic", 8.0, 8.0, -1, 49, 0.0, false, false, false)
		else
			-- If the animation is already playing, stop it
			StopAnimTask(playerPed, "anim@cop_mic_pose_002", "chat_mic", -4.0)
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if initialised then
			...
			if IsControlJustPressed(1, mumbleConfig.controls.proximity.key) then
			...
			if mumbleConfig.radioEnabled then
				if not mumbleConfig.controls.radio.pressed then
					if IsControlJustPressed(1, mumbleConfig.controls.radio.key) then
