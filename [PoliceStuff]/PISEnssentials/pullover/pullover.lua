...
-----------------------
	Citizen.CreateThread(function()
		...
		while true do Citizen.Wait(0)
			if IsPIS and IsControlJustPressed(0, kbpomnu) or IsControlJustPressed(0, ctrpomnu) then
			player = GetPlayerPed(-1)
...
RegisterNetEvent("PIS:conorner")
AddEventHandler("PIS:conorner", function(emptystring)

	for k, gaurd in pairs(emptystring) do
		if(gaurd ~= nil) then
			DeletePed(gaurd)
			emptystring[k] = nil
		end
